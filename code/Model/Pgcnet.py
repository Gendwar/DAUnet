...
class PGCNet(nn.Module):
    ...
    def forward(self, x):
        ...
        cell_sum=torch.sum(torch.sum(output,dim=3),dim=2)
        return output,cell_sum
...
def creat():
    ...
    channel_attention={'name':'FC', 'channel_attention_params':{'vector_dimension':16}}
    caff_enable=[0,0,1]
    caff={'name': 'ICA',
      'channel_attention_params':{'vector_dimension': 16}}
    bil=[[0,1], [0,0], [0,0]]
    gradient_initial= [1, 1, 1]
    gradient_func= None


    model=PGCNet(bil=bil, caff=caff, caff_enable=caff_enable,
                channel_attention=channel_attention, channel_attention_enable=channel_attention_enable, 
                gradient_initial=gradient_initial, gradm_stategylist=None)
